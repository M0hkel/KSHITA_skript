...
if (-not $ResumePhase) {
    ...
    try {
        ...
        $cmd = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$scriptPath`" -ResumePhase 2"
        New-ItemProperty -Path $runonceKey -Name "ResumeADDSPhase2" -Value $cmd -PropertyType String -Force | Out-Null
        Write-Output "RunOnce registry entry created to resume Phase 2."
    }
    catch { Write-Output "RunOnce registreerimise viga: $_" }

    try {
        Write-Output "Phase 1 complete. Rebooting system..."
        Restart-Computer -Force
    }
    catch { Write-Output "Taaskaivitamise viga: $_" }
    exit
}
...
if ($ResumePhase -eq "2") {
    ...
    Write-Output "Skripti taitmine on lopetatud."
}